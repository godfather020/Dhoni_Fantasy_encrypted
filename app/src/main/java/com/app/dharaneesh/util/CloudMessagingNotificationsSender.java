...
//            } else {
            ...
            Log.d("ttt", "sending to userid: " + data.getBody() + data.getImageUrl());

            usersRef.child(userId).get().addOnSuccessListener(new OnSuccessListener<DataSnapshot>() {
                @Override
                public void onSuccess(@NonNull DataSnapshot dataSnapshot) {
                    String token = dataSnapshot.child("cloudMessagingToken").getValue(String.class);
                    Log.d("ttt", "sending to token: " + token);
                    Sender sender = new Sender(data, token);
                    apiService.sendNotification(sender).enqueue(new Callback<MyResponse>() {
                        @Override
        ...
        @Headers(
                {
                        ...
                        //"Authorization:key=AAAA2ZFL7FE:APA91bHRlMNUkd7X7tAWWuoUSX-PSPu_0KfYYXK45eczoyR0RQNcuwkTKalsPq_ibKcKzIapX8gJyiHcXPrnOih_4p9eXC-uvm5LKRzrUNeaNBbA7WLjfA7u_u8Aim8odyMnqzKtiExS"
    ...
    public static class Data {
        ...
        public Data(Map<String, String> dataMap) {
            ...
            if (dataMap.containsKey("isschedule")) {
                this.destinationID = dataMap.get("isschedule");
            }
            if (dataMap.containsKey("time")) {
                this.destinationID = dataMap.get("time");
            }

        }

        public Data(String senderID, String title, String body, String imageUrl, boolean isschedule, String time, int type) {
